...
5.单纯形法：建表 计算检验数 找检验数最小的列（ak）进基，计算yi0/yik找到最小的（并且必须大于零）离基，直至检验数为0

6.对偶单纯形法：化标准形式（不要求b大于0）建表，求对偶可行的基本解，若xB中有负的分量把最小的离基，zj-cj/ykj（需ykj<0，yij表示单纯形表中第Bi行的各个元素，k其实是单纯形表中的第k行，也是上一步确定离基矢量时对应的一行） ，直到所有分量都大于0
